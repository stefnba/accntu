...
interface TreeRemoveProps extends TreeActionProps {
    itemLabel?: string;
}

export const TreeRemove = ({ itemLabel, ...props }: TreeRemoveProps) => {
    const ariaLabel = itemLabel 
        ? `${ACCESSIBILITY_LABELS.removeButton} ${itemLabel}`
        : ACCESSIBILITY_LABELS.removeButton;

    return (
        <TreeAction
            aria-label={ariaLabel}
            data-testid="tree-remove-button"
            active={{
            ...
            {...props}
        >
            <svg width="8" viewBox="0 0 22 22" xmlns="http://www.w3.org/2000/svg" className="fill-current">
