...
import { Preview } from '@/features/transaction-import/components/import-modal/preview';
import { useImportModal } from '@/features/transaction-import/hooks';
import { CheckCircle2, Clock, Loader2 } from 'lucide-react';
import { useEffect, useState } from 'react';

export const ProcessingAndPreviewStep: React.FC = () => {
    ...
    const { data, mutateAsync: initParse } = parseTransactions;

    useEffect(() => {
        if (!activeFileId) return;

        parseTransactions.mutate({ param: { id: activeFileId } });
        ...
        const processFile = async () => {
            try {
                ...
                await initParse({ param: { id: activeFileId } });
    ...
    if (parseTransactions.data) {
        return <Preview parseResult={parseTransactions.data} />;
    }
};
