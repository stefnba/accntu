...
export const clientEnvSchema = z.object({
    ...
    NEXT_PUBLIC_APP_URL: z.string().url({ error: 'NEXT_PUBLIC_APP_URL must be a valid URL' }),
...
export const authEnvSchema = z.object({
    ...
    GITHUB_CLIENT_SECRET: z.string().min(1, { message: 'GITHUB_CLIENT_SECRET is required' }),
...
const baseEmailEnvSchema = z.object({
    ...
    EMAIL_FROM_ADDRESS: z.string().email({ error: 'EMAIL_FROM_ADDRESS must be a valid email' }),
});

const resendEmailEnvSchema = baseEmailEnvSchema.extend({
    EMAIL_PROVIDER: z.literal('resend'),
    RESEND_API_KEY: z.string().min(1, { message: 'RESEND_API_KEY is required for resend provider' }),
});

const smtpEmailEnvSchema = baseEmailEnvSchema.extend({
    EMAIL_PROVIDER: z.literal('smtp'),
    SMTP_HOST: z.string().min(1, { message: 'SMTP_HOST is required for smtp provider' }),
    SMTP_PORT: z.coerce.number().min(1).max(65535, { message: 'SMTP_PORT must be between 1 and 65535' }),
...
const mailtrapEmailEnvSchema = baseEmailEnvSchema.extend({
    ...
    MAILTRAP_HOST: z.string().min(1, { message: 'MAILTRAP_HOST is required for mailtrap provider' }),
    MAILTRAP_PORT: z.coerce.number().min(1).max(65535, { message: 'MAILTRAP_PORT must be between 1 and 65535' }),
    MAILTRAP_USER: z.string().min(1, { message: 'MAILTRAP_USER is required for mailtrap provider' }),
    MAILTRAP_PASS: z.string().min(1, { message: 'MAILTRAP_PASS is required for mailtrap provider' }),
});
...
export const awsEnvSchema = z.object({
    AWS_BUCKET_REGION: z.string().min(1, { message: 'AWS_BUCKET_REGION is required' }),
