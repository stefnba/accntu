...
// For backward compatibility, provide functions that accept the old parameter style
// but convert them to the new object parameter style
export const createApiError = (
    message: string,
    code: ErrorCode,
    cause?: Error,
    statusCode: ContentfulStatusCode = 400
) => errorFactory.createApiError({ message, code, cause, statusCode });

export const createAuthError = (
    message: string,
    code: ErrorCode,
    statusCodeOrCause?: ContentfulStatusCode | Error,
    statusCode?: ContentfulStatusCode
) => {
    // Handle the case where the third parameter might be a status code or a cause
    let cause: Error | undefined;
    let actualStatusCode: ContentfulStatusCode | undefined;

    if (typeof statusCodeOrCause === 'number') {
        actualStatusCode = statusCodeOrCause;
    } else if (statusCodeOrCause instanceof Error) {
        cause = statusCodeOrCause;
        actualStatusCode = statusCode;
    }

    return errorFactory.createAuthError({ message, code, cause, statusCode: actualStatusCode });
};
...
) => errorFactory.createDatabaseError({ message, code, cause, statusCode });
...
) => errorFactory.createExternalError({ message, code, cause, statusCode });
...
export const createServiceError = (
    ...
    cause?: Error,
    statusCode: ContentfulStatusCode = 500
) => errorFactory.createServiceError({ message, code, cause, statusCode });
...
export const createValidationError = (
    ...
    statusCode: ContentfulStatusCode = 400,
    details?: Record<string, unknown>
) => errorFactory.createValidationError({ message, code, cause, statusCode, details });
